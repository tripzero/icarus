...
'''
Adapted from the Adafruit pwm python code: https://github.com/adafruit/Adafruit-Raspberry-Pi-Python-Code to
use mraa and match the mraa API.
...
class Pca9685 :
  ...
  general_call_i2c = (0x00)

  @classmethod
  def softwareReset(cls):
    "Sends a software reset (SWRST) command to all the servo drivers on the bus"
    cls.general_call_i2c.writeReg(0x06)        # SWRST
  ...
  def __init__(self, address=0x40, debug=False):
    ...
    mode1 = self.i2c.readByte(self.__MODE1)
  ...
  def setPWMFreq(self, freq):
    ...
    oldmode = self.i2c.readByte(self.__MODE1);
 ...
 class Pwm(Pca9685):
 	...
 	def period(self, seconds):
 		self.setPWMFreq(seconds)

 	def write(self, percentage):
 		self.setPwm(channel, 0, percentage * 4096)
